...
def test_trusted_incoming_degrees_count_only_exact_and_scoped(tmp_path: Path) -> None:
    index = _build_two_file_index(tmp_path)
    ...
    with index.read_session() as reads:
        top = reads.trusted_incoming_degrees(10)
    # sym-a1's only incoming reference is heuristic and must not count.
    assert [(symbol.id, count) for symbol, count in top] == [("sym-b1", 2)]


def test_containment_and_import_projections_are_deterministic(tmp_path: Path) -> None:
    index = _build_two_file_index(tmp_path)
    with index.read_session() as reads:
        children = reads.containment_child_counts(100)
        imports = reads.import_name_counts()
    assert children == {}
    assert imports == {"json": 1, "os": 1, "pathlib.Path": 1}
